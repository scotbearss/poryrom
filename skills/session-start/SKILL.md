---
name: session-start
description: Orient at the start of a session working on a GBA ROM hack — check the machine can build, confirm the game's backup is current, and report where things stand in plain English. Use when opening a game repo, when the user says "where were we", "what's the state", "let's get going", or before starting any build work in a rombench game repo.
---
...
1. **`python3 "$ROMBENCH/tools/doctor.py"`** — can this machine build right now? It checks
...
2. **`python3 "$ROMBENCH/tools/export_hack.py" <game> --check`** — is the game's backup
...
`$ROMBENCH` is this plugin's directory. If the variable is not set, find it — the tools live
