...
| **The type chart** | dense **21×21**, direct-indexed | 🔇 A literal 441-cell array, every cell written out — **adding a type is a table-width change, not an append**: all 21 rows must gain a column *and* a new row must be added, or the array is ragged and **the compiler zero-fills it, silently manufacturing immunities**. Index 10 (`TYPE_MYSTERY`) is a hole in the battle-type set, so a table-driven tool cannot assume the indices are contiguous battle types. And the table is not the system: **nine mechanisms override a cell before use and seven more override the product** — Levitate is not in the table at all. The product also rounds half **UP** where the damage chain rounds half down; every shipped cell is a power of two so nothing shows it, but a non-power-of-two multiplier (a hack adding 0.75×, say) exposes the difference immediately. | 48 §2.1–2.6 |
...
**Records distilled here:** 48 (the type chart), 67 (the slate, which tabulates them), 68 (map size, spawn window), 69 (Poké Ball item ids), 74 (OBJ palette), 76 (audio ceiling), 77 (metatile behaviors, tilesets, field moves), 78 (battle-script stack, `BattleScripting`), 79 (box arena, budgets), plus 41/42/47/51 for the read-not-run figures they carry.
